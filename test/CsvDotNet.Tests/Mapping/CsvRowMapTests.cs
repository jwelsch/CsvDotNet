...
namespace CsvDotNet.Tests.Mapping
{
    public class CsvRowMapTests
    {
        ...
        [Fact]
        public void When_ordered_csv_row_contains_valid_data_for_type_then_return_type()
        {
            var id = AutoFixture.Create<int>();
            var name = AutoFixture.Create<string>();
            var timestamp = AutoFixture.Create<DateTime>();
            ...
            var sampleRow = new OrderedSampleRow();

            var typeFactory = Substitute.For<ITypeFactory>();
            typeFactory.Create<OrderedSampleRow>().Returns(sampleRow);
            ...
            var map = new CsvRowMap<OrderedSampleRow>(typeFactory, typeDeserializer, maps);

            var row = map.Map(csvRow);

            row.GetType().Should().Be(typeof(OrderedSampleRow));
        ...
        [Fact]
        public void When_ordered_csv_row_does_not_contain_valid_data_for_type_then_throw_exception()
        {
            var id = AutoFixture.Create<Guid>();
            var name = AutoFixture.Create<string>();
            var timestamp = AutoFixture.Create<DateTime>();
